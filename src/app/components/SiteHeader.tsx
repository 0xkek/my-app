// src/components/SiteHeader.tsx (Stable State - text-lg Nav Links)
'use client';
...
export function SiteHeader() {
  ...
  const getLinkClass = (path: string) => { const isActive = (path === '/' && pathname === path) || (path !== '/' && pathname.startsWith(path)); return isActive ? 'text-white font-semibold' : 'text-slate-400 hover:text-slate-200'; };
      ...
      <div className="container mx-auto flex flex-wrap items-center justify-between p-4 gap-y-2">
        ...
        <div className="flex items-center gap-x-4 sm:gap-x-6">
          <nav className="flex flex-wrap gap-x-4 sm:gap-x-6 gap-y-1 items-center">
            ...
            <Link href="/about" className={`${baseLinkClass} text-lg ${getLinkClass('/about')}`}>About</Link>
            <Link href="/blog" className={`${baseLinkClass} text-lg ${getLinkClass('/blog')}`}>Blog</Link>
            <Link href="/projects" className={`${baseLinkClass} text-lg ${getLinkClass('/projects')}`}>Projects</Link>
          </nav>
          <div>
